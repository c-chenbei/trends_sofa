    ...
    (function($) {
        ...
        var IAS = function($element, options) {
            ...
            if(options.multi){
                ...
                this.$itemsContainer = $element.find(this.itemsContainerSelector);
            }else {
            ...
            this.getLastItem = function() {
                let itemLatest = $(this.itemSelector, this.$itemsContainer.get(0)).last();
                if(!itemLatest.length && this.smart){
                    itemLatest = $(this.$itemsContainer.get(0)).children().not('[id^="ias_spinner_"]').last();
                }
                return itemLatest;
            };
            ...
            this.getFirstItem = function() {
                let itemFirst = $(this.itemSelector, this.$itemsContainer.get(0)).first();
                if(!itemFirst.length && this.smart){
                    itemFirst = $(this.$itemsContainer.get(0)).children().first();
                }
                return itemFirst;
            };
            ...
            this.getNextUrl = function(container) {
                ...
                if(!next_url){
                    $(this.paginationSelector, container).find('[href]').each(function(){
                        let href   = $(this).attr('href');
                        let params = new URLSearchParams(this.search);
                        let page   = params.get("page") ? params.get("page") : params.get("p");
                    ...
                    if(!next_url){
                        var customNext = $(this.paginationSelector);
                        if(customNext.data('custom-next-page')){
                            next_url = customNext.data('custom-next-page');
                        }
                    }
                }
                if(next_url) {
            ...
            this.getNextUrlShopify = function(handle) {
                let next_url;
                /**
                * The graphql query
                **/
            ...
            this.load = function(url, callback, delay) {
                ...
                return $.get(loadEvent.url, { "_": $.now() }, $.proxy(function(data) {
                    ...
                    if ($itemContainer) {
                        let $items = $itemContainer.find(this.itemSelector);
                        if(!$items.length && this.smart) $items = $itemContainer.children();
                        $items.each(function() {
                            items.push(this);
                        });
                    }
